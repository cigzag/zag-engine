...
// Keep track in our memory subsystem.
struct memory_stats {
    u64 total_allocated;
...
char* get_memory_usage_str() {
    ...
    u64 offset = strlen(buffer); // Sizeof buffer string

    for(u32 i = 0; i < MEMORY_TAG_MAX_TAGS; ++i) {
        char unit[4] = "XiB";
        float amount = 1.0f;
        // Set size tag to correct unit.
    ...
    // Ensure to free memory, dynamic memory eww.
